...
public class ResourceCountsDialog extends JDialog implements ActionListener, ItemListener
{
	...
	private TreeMap<NetNode, List> counts;
	private JCheckBox filterNonResOpt;
	private JCheckBox filterZerosOpt;
	private JButton exportXls;


	public ResourceCountsDialog(Frame parent, AbstractPlan absPlan)
	{
		super(parent);
		setSize(400, 500);
		...
		try{
			...
			{
				counts = ((ResourcedLHSampler)samp).getEnsembleCounts();
	...
	{
		propTable = new ResCountsTable(model = new CountsTableModel(counts, filterNonResOpt.isSelected(), filterZerosOpt.isSelected()), true);
