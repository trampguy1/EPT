...
<xsl:template name ="createPlanItemEvent">
	...
	<PlanItem>
		...
		<Causes/>
		<Effects>
			<xsl:for-each select="descendant::node()[name()='child']">
				<Mech>
					<!--Will be the correct ID since 'generate-id()' is not random, 
					it generates an ID based on the position in the source tree-->
...
<xsl:template name ="createPlanItemMechanism">
	...
	<PlanItem>
		...
		<ToEvent><xsl:value-of select="child::node()[name()='child']/attribute::node" /></ToEvent> 
		<FromEvent><xsl:value-of select="child::node()[name()='parent']/attribute::node" /></FromEvent> 
		<Delay>0</Delay> 
		<Persistence>1</Persistence> 
		<Continuation>0.0f</Continuation>
	</PlanItem>
